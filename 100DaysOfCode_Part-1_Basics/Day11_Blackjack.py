############### Blackjack Project #####################


############### Our Blackjack House Rules #####################

## The deck is unlimited in size. 
## There are no jokers. 
## The Jack/Queen/King all count as 10.
## The the Ace can count as 11 or 1.
## Use the following list as the deck of cards:
## cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
## The cards in the list have equal probability of being drawn.
## Cards are not removed from the deck as they are drawn.
## The computer is the dealer.

##################### Hints #####################

#Step 1: Go to this website and try out the Blackjack game: 
#   https://games.washingtonpost.com/games/blackjack/
#Then try out the completed Blackjack project here: 
#   http://blackjack-final.appbrewery.repl.run

#Step 2: Read this breakdown of program requirements: 
#   http://listmoz.com/view/6h34DJpvJBFVRlZfJvxF
#Then try to create your own flowchart for the program.

#Step 3: Download and read this flow chart I've created: 
#   https://drive.google.com/uc?export=download&id=1rDkiHCrhaf9eX7u7yjM1qwSuyEk-rPnt

#Step 4: Create a deal_card() function that uses the List below to *return* a random card.
#11 is the Ace.

#Step 5: Deal the user and computer 2 cards each using deal_card() and append().
#user_cards = []
# #computer_cards = []

#Step 6: Create a function called calculate_score() that takes a List of cards as input 
#and returns the score. 
#Look up the sum() function to help you do this.

#Step 7: Inside calculate_score() check for a blackjack (a hand with only 2 cards: ace + 10) and return 0 instead of the actual score. 0 will represent a blackjack in our game.

#Step 8: Inside calculate_score() check for an 11 (ace). If the score is already over 21, remove the 11 and replace it with a 1. You might need to look up append() and remove().

#Step 9: Call calculate_score(). If the computer or the user has a blackjack (0) or if the user's score is over 21, then the game ends.

#Step 10: If the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_card() function to add another card to the user_cards List. If no, then the game has ended.

#Step 11: The score will need to be rechecked with every new card drawn and the checks in Hint 9 need to be repeated until the game ends.

#Step 12: Once the user is done, it's time to let the computer play. The computer should keep drawing cards as long as it has a score less than 17.

#Step 13: Create a function called compare() and pass in the user_score and computer_score. If the computer and user both have the same score, then it's a draw. If the computer has a blackjack (0), then the user loses. If the user has a blackjack (0), then the user wins. If the user_score is over 21, then the user loses. If the computer_score is over 21, then the computer loses. If none of the above, then the player with the highest score wins.

#Step 14: Ask the user if they want to restart the game. If they answer yes, clear the console and start a new game of blackjack and show the logo from art.py.



import random
import art
from replit import clear

play_game = True


while play_game:
  another_game = input("Do you want to play a game of Blackjack? Type 'y' or 'n': ")
  if another_game == "n":
    play_game = False
  else:
    clear()
    print(art.logo)
    
    """Deals a card"""
    def deal_cards():
      card = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
      deal_card = random.choice(card)
      return deal_card
    
    
    """Take a list of cards and calculate the score from the cards in list"""
    def calculate_score(list_name):
      if (sum(list_name) == 21) and len(list_name) == 2:
        return 0
      if sum(list_name) > 21 and 11 in list_name:
        list_name.remove(11)
        list_name.append(1)
      return sum(list_name)

    
    yourcard = []
    computercard = []
    """deals 2 cards each for both user and computer"""
    for i in range(2):
      yourcard.append(deal_cards())
      computercard.append(deal_cards())
    
    should_continue = True
    
    while should_continue:
      user_score = calculate_score(yourcard)
      computer_score = calculate_score(computercard)
      print(f"your cards {yourcard} and your score: {user_score}")
      print(f"computer's first card {computercard[0]}")
      if user_score > 21 or user_score == 0 or computer_score == 0:
        should_continue = False
      else:
        another_card = input("Would you like to draw another card? type y or n: ")
        if another_card == "y":
          yourcard.append(deal_cards())
        else:
          should_continue = False
          
    
    while computer_score != 0 and computer_score < 17:
      computercard.append(deal_cards())
      computer_score = calculate_score(computercard)
    
    def compare(user_score, computer_score):
      if user_score == computer_score:
        return "it's a draw"
      elif computer_score == 0:
        return "computer wins. Computer has a blackjack"
      elif user_score == 0:
        return "User wins. User has a blackjack"
      elif user_score > 21:
        return "Computer wins. User score greater than 21."
      elif computer_score > 21:
        return "user wins. computer score greater than 21."
      elif user_score > computer_score:
        return "User has highest score and user won"
      else:
        return "computer has highest score and computer won"
    
    print(f"Your final hand: {yourcard}, final score: {user_score}")
    print(f"Computer's final hand: {computercard}, final score: {computer_score}")
  
    print(compare(user_score, computer_score))
    